...
## Proteger el panel de administración con Firebase (correo y contraseña)

El link "Admin" ya no aparece en el menú público — la única forma de llegar es
escribiendo `tuusuario.github.io/LRDG_TV/#/admin` directamente en el navegador. Además,
esa ruta ahora pide iniciar sesión con correo y contraseña antes de mostrar nada.

### 1. Crear el proyecto de Firebase
1. Ve a [console.firebase.google.com](https://console.firebase.google.com) → **Agregar proyecto**.
2. Ponle un nombre (ej. `lrdg-tv`) y sigue el asistente. Puedes desactivar Google Analytics, no lo necesitas.

### 2. Activar el inicio de sesión con correo/contraseña
...
2. Escribe tu correo y una contraseña segura. Ese será el único usuario que puede
   entrar al panel — no hay registro público, solo tú puedes iniciar sesión.

### 4. Registrar la app web y obtener las claves
1. Ve a **⚙️ Configuración del proyecto → Tus apps** → clic en el ícono `</>` (Web).
2. Dale un nombre (ej. "LRDG_TV Web") → **Registrar app**. No necesitas Firebase Hosting.
3. Copia el objeto `firebaseConfig` que te muestra.

### 5. Pegar la configuración en el proyecto
Abre `src/firebase.js` y reemplaza los valores de ejemplo con los tuyos:
...
> Estas claves no son secretas: Firebase está diseñado para que viajen al navegador.
> La seguridad la da el inicio de sesión y el hecho de que solo tú tienes una cuenta
> creada — no hace falta ocultarlas ni usar variables de entorno.

### 6. Autorizar tu dominio de GitHub Pages
1. En Authentication → **Settings → Authorized domains** → **Add domain**.
2. Agrega `tuusuario.github.io` (sin `/LRDG_TV/`, solo el dominio).

### 7. Instalar la dependencia y publicar
```bash
npm install
git add .
git commit -m "Proteger admin con Firebase Auth"
git push
```

Ahora entra a `tuusuario.github.io/LRDG_TV/#/admin`, inicia sesión con tu correo y
contraseña, y verás el panel. Hay un botón **"Cerrar sesión"** arriba a la derecha.
